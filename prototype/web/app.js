(() => {
  ...
  const NOTE_LABELS = {
    ...
    LEADERS_CARRY: "У части похожих товаров продажи высокие, но у большинства — ниже",
  ...
  function renderResult(d) {
    ...
    // Дублируем предупреждение о широком подборе прямо здесь: верхнее (scope-note)
    // легко потерять — карточки ниже в свёрнутом «Подробнее», и без явной пометки
    // выглядят как точные аналоги, хотя вид товара не подтверждён.
    const exScopeWarn = $("examples-scope-warn");
    if (d.niche_scope === "type") {
      exScopeWarn.textContent = "⚠ Вид товара не подтверждён — это широкая категория, карточки ниже могут визуально не совпадать с фото";
      exScopeWarn.classList.remove("hidden");
    } else {
      exScopeWarn.classList.add("hidden");
    }
    ex.innerHTML = "";
